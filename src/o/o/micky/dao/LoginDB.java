...
public class LoginDB
{
	...
	{														//获得数据库连接的方法
		...
		/*	Context initial=new InitialContext();			//获得上下文
			DataSource ds=(DataSource)initial.lookup("java:comp/env/rlzy");
			conn=ds.getConnection();*/						//通过数据源获得数据库连接
			Class.forName("com.mysql.jdbc.Driver");
			conn = DriverManager
					.getConnection(
							"jdbc:mysql://localhost:3306/table_in_test?useUnicode=true&characterEncoding=GBK",
							"root", "123");
		}
		catch(ClassNotFoundException e){e.printStackTrace();}		//异常的捕获处理语句
